<template>
  <div v-if="latlng" class="drawer drawer--completed">
    <span class="drawer__label mr-2">Местоположение:</span> {{ latlng.lat }},
    {{ latlng.lng }}
    <div class="drawer__help-text caption">
      Вы можете изменить местоположение, подвинув метку на карте
    </div>
  </div>
  <bordered-block
    v-else
  ...
  methods: {
    ...
    createArrow() {
      ...
      this.arrow.innerHTML =
        "<svg x='0px' y='0px' width='196.9px' height='95.2px' viewBox='0 0 196.9 95.2' style='enable-background:new 0 0 196.9 95.2;' xml:space='preserve' fill='rgb(250,49,9)'>\
