<template>
  <div class="feedback-form" @scroll="onFormScroll($event)">
    <div class="feedback-form__header">
      <v-icon class="feedback-form__closer" @click="triggerClose()"
        >close</v-icon
      >
      ...
      <v-stepper v-model="feedbackStep" vertical>
        ...
        <v-stepper-content step="1">
          <v-btn
            color="primary"
            depressed
            :disabled="!formValues.theme || !formValues.title"
        ...
        <v-stepper-step step="2" :complete="feedbackStep > 2">
          <drawer
            class="feedback-form__set-location"
        ...
        <v-stepper-content step="2">
          <v-btn
            color="primary"
            depressed
            :disabled="!formValues.latlng"
            @click="feedbackStep = 3"
            >Продолжить</v-btn
          >
        </v-stepper-content>
    ...
    <div class="feedback-form__footer">
      ...
      <v-progress-circular
        indeterminate
        class="feedback-form__loader accent--text"
        v-if="formInProgress"
      ></v-progress-circular>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      formValid: false,
      messageTypes: ['Нарушение', 'Предложение', 'Природный объект'],
      formInProgress: false,
    };
  },
  computed: {
    ...mapState(['themes', 'selectedThemeId', 'user']),
    themeList() {
    ...
    editableLayer() {
      return this.selectedThemeId != undefined
        ? this.themes[this.selectedThemeId].editableLayer
        : undefined;
    },
  },
  watch: {
    ...
    themes(val, oldVal) {
      if (!oldVal.length)
        this.$set(
          this.formValues,
          'theme',
          this.themes[this.selectedThemeId].name,
        );
    },
  },
  mounted() {
  ...
  methods: {
    ...
    onThemeChanged(value) {
      let selectedThemeId = this.themes.findIndex((theme) => {
      ...
      if (!this.formInProgress) {
        ...
        axios
          .post(
            nextgiscomUrl +
              '/api/resource/' +
              this.editableLayer.resource.id +
              '/feature/',
            JSON.stringify({
    ...
    uploadFiles(featureId) {
      ...
      return new Promise(function(resolve, reject) {
        if (that.formValues.file.length) {
          let files = that.formValues.file.map((item) => {
              return item.file;
            }),
          ...
          axios
            .post(
              nextgiscomUrl + '/api/component/file_upload/upload',
              formData,
              {
                headers: {
                  Authorization: 'Basic ' + localStorage.getItem('clientId'),
              ...
              uplodedFiles.forEach(function(file) {
                ...
                axios
                  .post(
                    nextgiscomUrl +
                      '/api/resource/' +
                      that.editableLayer.resource.id +
                      '/feature/' +
                      featureId +
                      '/attachment/',
                    JSON.stringify(attach_data),
                    {
                      headers: {
                        Authorization:
                          'Basic ' + localStorage.getItem('clientId'),
                      },
                    },
                  )
                  .then(function(res) {
