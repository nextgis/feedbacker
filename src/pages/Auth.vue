    ...
    <v-container class="auth__inner ma-0">
      <router-link class="nolink" to="/">
        <div class="auth__logo-wrapper">
          <img
            class="auth__logo"
            src="../assets/ng_logo.svg"
            alt="Feedbacker"
          />
        </div>
      </router-link>
      <router-link class="nolink" to="/"
        ><h1 class="auth__title">Feedbacker</h1></router-link
      >
      <form name="auth-form" @submit.prevent="submitForm()">
        ...
        <v-btn light depressed class="btn--xlarge mt-3" type="submit"
          >Войти</v-btn
        >
      </form>
    </v-container>
...
export default {
  ...
  computed: {
    ...mapState(['user', 'previousRoute']),
  },
  methods: {
    submitForm() {
      if (this.login && this.password) {
        let clientId = window.btoa(
          unescape(encodeURIComponent(this.login + ':' + this.password)),
        );
        this.$store
          .dispatch('getUserData', clientId)
          .then((userData) => {
    ...
    close() {
      if (this.previousRoute) {
        this.$router.push({
          ...
          query: Object.assign({}, this.previousRoute.query, {
            feedback: undefined,
          }),
        });
      } else {
