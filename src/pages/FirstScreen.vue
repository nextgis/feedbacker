<template>
  <div
    class="first-screen"
    :class="{
      'first-screen--withGreeting':
        projectDescription && projectDescription.length,
    }"
  >
    <div
      class="first-screen__greeting"
      v-if="projectDescription && projectDescription.length"
    >
      <v-container fluid>
        <span
          class="first-screen__greeting-text"
    ...
    <div class="first-screen__themes">
      <v-container fluid grid-list-md>
        <h1 class="first-screen__title">
          Выберите тему, чтобы оставить сообщение
        </h1>
        <theme-list
...
export default {
  ...
  computed: mapState(['themes', 'projectDescription']),
  mounted() {
